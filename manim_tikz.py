...
from manim_tikz_template import TikzTemplate
...
class Tikz(SVGMobject):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        file_name = self.convert(
            code, packages, libraries, tikzset, preamble, use_pdf
        )
        super().__init__(
            file_name,
            **kwargs,
    ...
    def convert(
        self, code, packages, libraries, tikzset, preamble, use_pdf
    ) -> str:
        return tex_to_svg_file(
            code,
            environment="tikzpicture",
            tex_template=TikzTemplate(
                packages, libraries, tikzset, preamble, use_pdf
            ),
        )
