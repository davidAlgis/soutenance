...
def slide_17(self):
    ...
    Includes 1D and 2D wave equation animations (No Damping -> Damping).
    Refactored to use pure Manim/Numpy (No Matplotlib).
    """
    # --- Barre de titre -------------------------------------------------------
    ...
    # 2. Helper: Create 1D Animation Group (Classical Manim Line + Fill)
    def create_1d_group(data, title_str):
        ...
        source_dot = Dot(axes.c2p(0, A), color=UCLA_GOLD_HEX, radius=0.08)
        label = Text(title_str, font_size=20, color=BLACK).next_to(
            bg_square, DOWN, buff=0.2
        )

        group = VGroup(
            bg_square, axes, wave_area, wave_line, source_dot, label
        )
        ...
        def update_wave(mob, dt):
            ...
            bottom_left = np.array([x_coords[0], base_y - 2.0, 0])
            bottom_right = np.array([x_coords[-1], base_y - 2.0, 0])
    ...
    # 3. Helper: Create 2D Animation Group (Numpy Color Map)
    def create_2d_group(data, title_str):
        ...
        source_dot = Dot(color=UCLA_GOLD_HEX, radius=0.08).move_to(bg_square)
        label = Text(title_str, font_size=20, color=BLACK).next_to(
            bg_square, DOWN, buff=0.2
        )
        ...
        def get_img_from_index(idx):
            arr = H[idx].T
            arr = np.flipud(arr)  # Correct orientation
        ...
        group = Group(bg_square, img_mobj, source_dot, label)
    ...
    # FIX: We MUST wait here to let the updaters run and generate video frames
    self.next_slide(loop=True)  # Loop this section in presentation
    self.wait(5)  # Generate 5 seconds of animation
