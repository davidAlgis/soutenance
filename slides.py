...
class Presentation(Slide):
    ...
    def slide_06(self):
        ...
        # ========= Left: text column =========
        # Headings + lists
        h1 = Text("Unity un moteur de jeu polyvalent :", color=BLACK,
                  font_size=self.BODY_FONT_SIZE, weight=BOLD)
        ...
        h2 = Text("Les avantages et les inconvénients :", color=BLACK,
                  font_size=self.BODY_FONT_SIZE, weight=BOLD)
        h2.align_to(h1, LEFT)
        ...
        left_group.move_to([x_left + 0.2 + left_group.width * 0.5, y_top - 0.2 - left_group.height * 0.5, 0])

        # Auto-scale if too wide or too tall
        if left_group.width > max_left_w or left_group.height > max_left_h:
            s = min(max_left_w / left_group.width, max_left_h / left_group.height, 1.0)
